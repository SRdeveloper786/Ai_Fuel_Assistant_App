...
export default function CheapFuelFinder({ activeVehicle, currency = "PKR" }: CheapFuelFinderProps) {
  ...
  return (
    <div className="space-y-4">
      {/* ⚠️ Dynamic Banner for Google Maps API Key Setup */}
      {!hasValidKey && (
        <div className="bg-indigo-950/40 border border-indigo-500/30 rounded-3xl p-5 space-y-4 shadow-xl">
          <div className="flex items-start gap-3">
            <div className="p-2.5 bg-indigo-500/10 rounded-2xl border border-indigo-500/20 text-indigo-400 shrink-0 mt-0.5">
              <Sparkles size={18} className="animate-pulse" />
            </div>
            <div className="space-y-1">
              <h3 className="text-sm font-extrabold text-slate-100 uppercase tracking-wider">
                🌟 Real Google Maps Integration Available
              </h3>
              <p className="text-xs text-slate-400 leading-relaxed">
                We've built a fully integrated Google Map with real-time Routes API polyline drawing, direction computing, and Advanced Markers. The app is currently running in **Simulated Interactive Sandbox Mode** because a Maps API Key is not set in your secrets yet.
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-950/50 p-4 rounded-2xl border border-slate-800/60">
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-300 flex items-center gap-1.5 uppercase">
                ...
                How to activate real Google Maps:
              </h4>
              <ol className="text-[11px] text-slate-400 space-y-2 leading-relaxed list-decimal list-inside pl-1">
                <li>
                  Get a standard Google Maps API Key at{" "}
                  <a
                ...
                <li>
                  Click the ⚙️ <strong>Settings (Gear icon)</strong> at the top right corner of AI Studio.
                </li>
                <li>
                  Select <strong>Secrets</strong>, type <code>GOOGLE_MAPS_PLATFORM_KEY</code> as the secret name, and paste your API key as the value. Press Enter!
                </li>
                <li>
                  The application will instantly re-build itself in the background and load the actual interactive satellite Google Map canvas!
                </li>
              </ol>
            </div>

            <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-3 flex flex-col justify-between">
              <div>
                <p className="text-[11px] font-bold text-indigo-400 uppercase">Interactive Sandbox Mode active</p>
                <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                  In the meantime, feel free to interact with our beautiful live mock canvas below! You can select cities, search stations, sort by cheap price or distance, draw custom simulated driving routes, and listen to spoken navigation prompts.
                </p>
              </div>
              <button
                onClick={() => setShowMockInfo(!showMockInfo)}
                className="mt-3 w-full text-[10px] bg-slate-850 hover:bg-slate-800 text-slate-300 font-semibold py-1.5 px-3 rounded-lg border border-slate-800 transition cursor-pointer"
              >
                {showMockInfo ? "Hide Sandbox Details" : "Show Sandbox Details"}
              </button>
            </div>
          </div>
        </div>
